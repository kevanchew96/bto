...
html_legend <- "<img src='https://www.flaticon.com/svg/static/icons/svg/2987/2987903.svg'style='width:10px;height:10px;'>&nbsp School<br/>
...
addMarkers(data=schools, ~lon, ~lat,popup= ~Schools, label = ~Schools,
                 icon=makeIcon("School.png",iconWidth = 25, iconHeight =25), group="Schools") %>% #image source: https://pixabay.com/vectors/book-books-library-books-reading-2022464/
...
mop <- mop[!is.na(mop$num_of_units),]
mop$flat_type <- gsub("ROOM", "Room", mop$flat_type)
mop$flat_type <- gsub( " ", "-", mop$flat_type)
mop$`Project Name`<- gsub("\n","",mop$`Project Name`)
mop$ID <- paste0("M",seq(1:nrow(mop)))
mop$`Total Units` <- mop$num_of_units
mop
write.csv(mop,"MOP_new.csv")
...
mop
mop %>% select(ID, `Town/Name`, `Project Name`, `End of MOP`, `Flat Type`, `Year Completed/ Year to be Complete`, `Price (Predicted)`, `No. of Units`,lon, lat, Type)
```
```{r}
mop$`Flat Type` <- as.factor(mop$`Flat Type`)
levels(mop$`Flat Type`)
bto <- read.csv("BTO_new.csv")
bto <- bto %>% rename(`Month of Launch`=`Month.of.Launch`,`Town/Estate`=`Town.Estate`,                      `Estimated Flats`=`Estimated.Flats`,`Flat Type`=flat_type,`Price (Estimated)`=price) %>% 
    select(ID, `Month of Launch`, `Town/Estate`, `Flat Type`, `Estimated Flats`, `Price (Estimated)`, lon, lat, Type)
```
```{r}
bto <- read.csv("BTO_new.csv")
...
mop
resale$Room <- as.factor(resale$Room)
levels(resale$Room)
bto
```
```{r}
...
df <- as.data.frame(html_table(tab))
df <- df %>% select(X1,X3) %>% rename(`Postal District`=X1, `General Area`=X3) %>% .[2:29,]
df$`General Area` <- gsub("\r","",df$`General Area`)
df$`General Area` <- gsub("\n","",df$`General Area`)
df$`General Area` <- paste0(df$`General Area`,", Singapore")
df2 <- geocode(location = df[,2], output = "latlona", source = "google")
df <- df %>% cbind(df2)
df$`Postal District`<- as.integer(df$`Postal District`)
df
write.csv(df,"Districts.csv")
...
districts %>% filter(Combined=="1 - Raffles      Place, Cecil, Marina, People's Park")
...
mop$`Project ID` <- paste0(mop$`Project Name`," ",mop$`Flat Type`)
mop
resale <- read.csv("Resale_coords.csv")
...
labsmop <- lapply(seq(nrow(resale)), function(i) {
  paste0( "Type: ", resale[i, "Type"], "<br>",
          "ID: ", resale[i, "ID"], "<br>",
          "Town/Name: ", resale[i, "Town/Name"], "<br>",
          "Project Name: ", resale[i, "Project Name"], "<br>",
          "End of MOP: ", resale[i, "End of MOP"], "<br>",
          "Flat Type: ", resale[i, "Flat Type"], "<br>",
          "Year Completed/ Year to be Complete: ", resale[i, "Year Completed/ Year to be Complete"], "<br>",
          "Predicted Price: ", resale[i, "Predicted Price"],
          "No. of Units: ", resale[i, "No. of Units"])})
...
schools %>% filter(Schools=="Admiralty Primary School")
```